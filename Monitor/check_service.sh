...
check_apache() {
        ...
        elif [[ `systemctl --host root@10.1.1.4 is-active Apache_status` != 'active' ]] && [[ $apache_already_running == 'true' ]]; then
...
check_mysql() {
	...
	elif [[ `systemctl --host root@10.1.1.4 is-active Mysql_status` != "active" ]] && [[ $mysql_already_running == 'true' ]]; then
...
check_python() {
        ...
        elif [[ `systemctl --host root@10.1.1.4 is-active Python_status` != "active" ]] && [[ $python_already_running == 'true' ]]; then
...
check_java() {
        ...
        elif [[ `systemctl --host root@10.1.1.4 is-active Java_status` != "active" ]] && [[ $java_already_running == 'true' ]]; then
...
check_docker() {
        ...
        elif [[ `systemctl --host root@10.1.1.4 is-active docker` != "active" ]] && [[ $docker_already_running == 'true' ]]; then
